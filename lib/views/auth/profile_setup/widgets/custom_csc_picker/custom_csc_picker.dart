...
class CustomCSCPicker extends StatefulWidget {
  ...
  @override
  CustomCSCPickerState createState() => CustomCSCPickerState();
}

class CustomCSCPickerState extends State<CustomCSCPicker> {
  ...
  Future<void> setDefaults() async {
    if (widget.currentCountry != null) {
      ...
      // If states are hidden, we might want to get cities immediately
      if (!widget.showStates && widget.showCities) await getCities();
    }
  ...
  Future<List<String?>> getCities() async {
    ...
    // Logic specific to CustomCSCPicker:
    // If states are NOT shown, we collect cities from ALL states
    List<Region?> selectedStates;

    if (widget.showStates) {
      // Normal behavior: cities based on selected state
      var takeState = country.map((e) => e.state).toList();
      // Flatten states
      var allStates = [];
      for (var list in takeState) {
        if (list != null) allStates.addAll(list);
      }

      selectedStates = allStates
          .where((item) {
            var stateName =
                widget.countryStateLanguage ==
      ...
      // Custom behavior: getAll cities from all states of the country
      var takeState = country.map((e) => e.state).toList();
      selectedStates = [];
      for (var list in takeState) {
        if (list != null) selectedStates.addAll(list.cast<Region>());
      }
    }

    for (var selectedState in selectedStates) {
      // var stateCities = state?.map((item) => item.city).toList();
      var stateCities = selectedState?.city;

      if (stateCities != null) {
        if (!mounted) return _cities;
        setState(() {
          var citiesName = stateCities.map((item) => item.name).toList();
    ...
    // Remove duplicates if any (though usually unique per state, might duplicate across states if names collide?)
    // Using simple sort is fine.
  ...
  void _onSelectedCountry(String value) {
    ...
    setState(() {
      ...
      if (value != _selectedCountry) {
        _statesModels.clear();
        _cities.clear();
        _selectedState = widget.stateDropdownLabel.tr(
          widget.countryStateLanguage,
        );
        _selectedCity = widget.cityDropdownLabel.tr(
          widget.countryStateLanguage,
        );
        ...
        if (widget.showStates) {
          ...
          // If states are hidden, we need to populate cities immediately based on country
        ...
        widget.layout == Layout.vertical
            ? Column(
                ...
                children: <Widget>[
                  ...
                  // Modified: Show city dropdown if showCities is true, regardless of showStates
                  widget.showCities && (widget.showStates || !widget.showStates)
  ...
  Widget countryDropdown() {
    return DropdownWithSearch(
      ...
      placeHolder: widget.countrySearchPlaceholder.tr(
        widget.countryStateLanguage,
      ),
  ...
  Widget stateDropdown() {
    return DropdownWithSearch(
      ...
      placeHolder: widget.stateSearchPlaceholder.tr(
        widget.countryStateLanguage,
      ),
  ...
  Widget cityDropdown() {
    return DropdownWithSearch(
      ...
      disabled: _cities.isEmpty
          ? true
          : false, // Fixed logic: enable if cities are available
      items: _cities,
      isArabic: widget.countryStateLanguage == CountryStateLanguage.arabic,
      selectedItemStyle: widget.selectedItemStyle,
      dropdownHeadingStyle: widget.dropdownHeadingStyle,
